...
const AdminCampFixPage = () => {
  ...
  const { id } = useParams();
  const [error, setError] = useState(false);
  const [description, setDescription] = useState(""); // 빈 문자열로 초기화
  const [name, setName] = useState(""); // 빈 문자열로 초기화
  const [tel, setTel] = useState(""); // 빈 문자열로 초기화
  const [price, setPrice] = useState(""); // 빈 문자열로 초기화
  const [standardNum, setStandardNum] = useState(""); // 빈 문자열로 초기화
  const [maxNum, setMaxNum] = useState(""); // 빈 문자열로 초기화
  const [overCharge, setOverCharge] = useState(""); // 빈 문자열로 초기화
  const [imageUrls, setImageUrls] = useState([]); // 빈 배열로 초기화
  const [category, setCategory] = useState([]); // 빈 배열로 초기화
  const [updatedImages, setUpdatedImages] = useState([]); // 빈 배열로 초기화
  const [addr, setAddr] = useState(""); // 빈 문자열로 초기화
